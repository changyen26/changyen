...
export default function CompetitionsSection() {
  ...
  const getRankColor = (rank: string) => {
    if (rank.includes('1') || rank.includes('金')) return 'from-yellow-400 to-yellow-600';
    if (rank.includes('2') || rank.includes('銀')) return 'from-gray-400 to-gray-600';
    if (rank.includes('3') || rank.includes('銅')) return 'from-orange-400 to-orange-600';
          ...
          >
            {mockCompetitions.map((competition, index) => (
              ...
              >
                <div className={`w-1/2 ${index % 2 === 0 ? 'pr-8' : 'pl-8'}`}>
                  <Card className="p-8 group hover:shadow-2xl">
                    <div className="flex items-start justify-between mb-6">
                      <div className={`p-3 rounded-full bg-gradient-to-r ${getRankColor(competition.rank_position)} text-white shadow-lg`}>
                        <Trophy size={24} />
                      </div>
                    ...
                    <div className="flex items-center gap-4 mb-4">
                      <div className={`px-4 py-2 rounded-full bg-gradient-to-r ${getRankColor(competition.rank_position)} text-white font-medium`}>
                        <Medal size={16} className="inline mr-2" />
                        {competition.award_title}
                      </div>
                      
                      <div className="text-sm text-gray-500 bg-gray-100 px-3 py-1 rounded-full">
                        {competition.rank_position}
                      </div>
                    </div>
                ...
                >
                  <div className={`w-16 h-16 rounded-full bg-gradient-to-r ${getRankColor(competition.rank_position)} shadow-xl flex items-center justify-center`}>
                    <Trophy size={24} className="text-white" />
                  </div>
                  
                  {/* 脈衝效果 */}
                  <motion.div
                    className={`absolute inset-0 rounded-full bg-gradient-to-r ${getRankColor(competition.rank_position)} opacity-30`}
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
            <div>
              <div className="text-3xl font-bold text-yellow-600 mb-2">
                {mockCompetitions.filter(c => c.rank_position.includes('1') || c.award_title.includes('金')).length}
              </div>
            ...
            <div>
              <div className="text-3xl font-bold text-gray-600 mb-2">
                {mockCompetitions.filter(c => c.rank_position.includes('2') || c.award_title.includes('銀')).length}
              </div>
            ...
            <div>
              <div className="text-3xl font-bold text-orange-600 mb-2">
                {mockCompetitions.filter(c => c.rank_position.includes('3') || c.award_title.includes('銅')).length}
              </div>
